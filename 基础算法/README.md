...
1. 认真读题
    - 读懂题意，可以理解样例
    - 数据范围，决定使用什么数据类型（：**建议只使用 long long double、 数组开大一些**
    - 倒推题意，思考是什么时间复杂度
2. 骗分方法
    - if 样例输出
    - cout 可能答案输出
    - 暴力得分
    - if 数据范围分类输出、cout、暴力、结合
3. 确保提交文件的正确性
    - 文件读写错误 freopen()
    - 一定要注意编译环境，不少缺少头文件

4. 数据的初始化
    - 数据一定要看好是在哪初始化，是否需要反复初始化
    - 全局变量，std 定义的时候，都是已经初始化好的
...
6. 慎重的问题
    - 遇到除法，乘法的时候，一定要注意
    - 遇到小数的时候，一定要注意精度问题，如果可以尽量使用
    - 字符串的接收

7. 合理的安排考试时间
    - 保证每道题都有代码

- 自己在写代码常犯的错误，运算优先级问题，不要舍不得括号的使用 例如搜索时候的标记
- if (=) == 
- 良好的代码风格，代码注释
- 不要舍不得定义中间变量
- 输出的是 `YES` 还是 `Yes` 还是 `yes`
- 数组一定要注意自己的下标是 0 - (n -1) 还是 1 - n
```
